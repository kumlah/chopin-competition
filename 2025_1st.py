...
OUTPUT_FILE = Path("2025_1st.json")
...
def load_video_ids_from_competitors():
    """
    competitors.json から「第1」列に動画IDが入っているものだけを抽出し、
    ...
    if not video_ids:
        raise ValueError("competitors.json に第1ラウンドの動画IDが1件も見つかりませんでした。")
...
def load_existing_history():
    """既存の2025_1st.jsonを読み込んで、リストとして返す。なければ空リスト。"""
...
def main():
    ...
    # competitors.json から第1ラウンド動画ID一覧を取得
