...
JSON_PATH = Path("2025_2nd.json")
HTML_PATH = Path("2025_2nd.html")
...
def load_latest_videos():
    ...
    if not isinstance(data, list) or not data:
        raise ValueError("2025_2nd.json の最上位が空でないリストになっていないようです。")
...
def main():
    ...
    # ★ 第2ラウンド動画IDが入っている人だけ対象
    second_rounders = [c for c in competitors_raw if c.get("第2")]

    for comp in second_rounders:
        video_id = comp.get("第2", "")
    ...
    html.append(
        "    <title>ショパコン勝手にYouTube聴衆賞(非公式) | 2025第2次予選集計</title>"
    )
    html.append(
        '    <meta name="description" content="ショパン国際ピアノコンクール2025第2次予選のYouTube再生回数を個人的にまとめた非公式メモです。順位と関係なく伸びているコンテスタントの存在を可視化するためのページです。">'
    )
    ...
    html.append("      <h1>第19回(2025)ショパン国際ピアノコンクール 第2次予選再生数ランキング</h1>")
    html.append(
